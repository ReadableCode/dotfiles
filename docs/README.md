# Docs index

One topic per file, grouped by filename prefix. New docs take one of these
prefixes; nothing lives unprefixed.

## `repo_*` - How this repo constellation works

- [repo_client_credentials.md](repo_client_credentials.md) - Client Credentials Repos (`*_credentials`)
- [repo_deploy_configs.md](repo_deploy_configs.md) - Deploying Configs with the Manifest
- [repo_gmail_filters.md](repo_gmail_filters.md) - Gmail filters as code
- [repo_init_worktree.md](repo_init_worktree.md) - Initialising a git worktree
- [repo_philosophy.md](repo_philosophy.md) - Repo philosophy and boundaries

## `setup_*` - Setting up a machine, an app, or a tool

- [setup_android_termux.md](setup_android_termux.md) - Android Termux
- [setup_argon_case.md](setup_argon_case.md) - Argon One Cases
- [setup_calendar_board.md](setup_calendar_board.md) - Calendar board — side-by-side calendars TUI
- [setup_claude_github_mcp.md](setup_claude_github_mcp.md) - Claude Code GitHub MCP — multiple accounts
- [setup_docker.md](setup_docker.md) - Installing Docker
- [setup_git.md](setup_git.md) - Setup Git
- [setup_git_portable.md](setup_git_portable.md) - Setting up Git portable
- [setup_github_cli.md](setup_github_cli.md) - GitHub CLI
- [setup_go.md](setup_go.md) - Setup Go
- [setup_google_mcp.md](setup_google_mcp.md) - Google MCP — your own Calendar and Gmail in Claude Code
- [setup_kubernetes_k3s.md](setup_kubernetes_k3s.md) - Kubernetes K3S Mixed Cluster
- [setup_libreoffice.md](setup_libreoffice.md) - setup_libreoffice
- [setup_linux_workstation.md](setup_linux_workstation.md) - Setup Linux Workstation
- [setup_local_ai.md](setup_local_ai.md) - Setting Up Local AI
- [setup_mac_workstation.md](setup_mac_workstation.md) - Setup macOS Workstation
- [setup_mkdocs.md](setup_mkdocs.md) - MKDOCS
- [setup_msys2.md](setup_msys2.md) - MSYS2
- [setup_neovim.md](setup_neovim.md) - Neovim Setup Guide
- [setup_pandoc.md](setup_pandoc.md) - Installing and Using Pandoc
- [setup_power_bi.md](setup_power_bi.md) - Setup Power BI
- [setup_powershell.md](setup_powershell.md) - Powershell
- [setup_python.md](setup_python.md) - Setting Up Python
- [setup_python_old.md](setup_python_old.md) - Setting Up Python
- [setup_python_portable.md](setup_python_portable.md) - Setting up Python Portable
- [setup_rust.md](setup_rust.md) - Setup Rust
- [setup_slack_mcp.md](setup_slack_mcp.md) - Slack MCP — searching and reading a workspace from Claude Code
- [setup_stable_diffusion.md](setup_stable_diffusion.md) - Stable Diffusion
- [setup_syncthing.md](setup_syncthing.md) - Setup Syncthing
- [setup_t3_code.md](setup_t3_code.md) - Setup T3 Code
- [setup_tailscale.md](setup_tailscale.md) - Setup Tailscale
- [setup_tiktok_scraper.md](setup_tiktok_scraper.md) - setup_tiktok_scraper
- [setup_unison.md](setup_unison.md) - Unison
- [setup_ups_monitoring.md](setup_ups_monitoring.md) - Setup NUT (Network UPS Tools)
- [setup_vnc_server.md](setup_vnc_server.md) - Setup Tiger VNC
- [setup_vscode.md](setup_vscode.md) - Setting up Visual Studio Code
- [setup_windows_chocolatey.md](setup_windows_chocolatey.md) - Setting Up Chocolatey on Windows
- [setup_windows_msys64_weasyprint.md](setup_windows_msys64_weasyprint.md) - WeasyPrint + GTK3 + MSYS2 on Windows (No Admin Rights)
- [setup_windows_portable_userapps.md](setup_windows_portable_userapps.md) - Windows Portable Toolchain (userapps)
- [setup_windows_ssh_server.md](setup_windows_ssh_server.md) - Setting Up SSH Server
- [setup_windows_workstation_personal.md](setup_windows_workstation_personal.md) - Setup Windows Workstation
- [setup_xcode_beta.md](setup_xcode_beta.md) - Xcode Setup Guide (Beta Workflow)
- [setup_youtube_dl.md](setup_youtube_dl.md) - Youtube-dl
- [setup_zed.md](setup_zed.md) - Zed Editor Setup on macOS

## `homelab_*` - Homelab infrastructure

- [homelab_deployments.md](homelab_deployments.md) - Homelab Deployments
- [homelab_docker_registry.md](homelab_docker_registry.md) - Using a Local Docker Registry
- [homelab_hosts_access.md](homelab_hosts_access.md) - Homelab hosts: access facts an agent needs
- [homelab_postgres_app_conventions.md](homelab_postgres_app_conventions.md) - Shared Postgres / PostgREST App Conventions
- [homelab_unraid_drives.md](homelab_unraid_drives.md) - homelab_unraid_drives
- [homelab_unraid_mem_logging.md](homelab_unraid_mem_logging.md) - Loop logging Unraid from powershell ssh client

## `howto_*` - Techniques and reference notes

- [howto_finding_files.md](howto_finding_files.md) - Find Artifacts of Syncing Issues
- [howto_finding_strings_vscode.md](howto_finding_strings_vscode.md) - String Replacement with VSCode
- [howto_formatting.md](howto_formatting.md) - Formatting
- [howto_nicegui_quasar_layers.md](howto_nicegui_quasar_layers.md) - NiceGUI 3.x and Quasar cascade layers
- [howto_rsync.md](howto_rsync.md) - Rsync Info
- [howto_searching_replacing.md](howto_searching_replacing.md) - Replacements
- [howto_symlinks_and_hardlinks.md](howto_symlinks_and_hardlinks.md) - Sym Linking and Hard Linking
- [howto_vscode_cleanup.md](howto_vscode_cleanup.md) - VS Code Cleanup

## `plan_*` - Design plans

- [plan_desk_dev_topology.md](plan_desk_dev_topology.md) - Plan: Desk and Dev Topology
- [plan_unified_cli_tui.md](plan_unified_cli_tui.md) - Unified CLI and TUI (design plan)
- [plan_unified_device.md](plan_unified_device.md) - Crafting a Unified Device Ecosystem: The Agnostic Approach

## Directories

- `assets/` - calendar_board_agenda.png, calendar_board_grid.png, calendar_board_once.png
- `setup_pi_lcd_screen/` - lcd_raspberry_pi_setup.md, tft_screen_pi_DFR0428_web.pdf
