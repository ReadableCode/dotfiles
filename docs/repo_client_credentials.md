...
| `acme_app_lists.yaml` | Optional **context app list** - package manager to package names (`choco: [slack]`, `cask: [slack]`), the apps only this context's machines get. Read by `src/app_lists.py`, which every installer in `scripts/` asks; loaded only where the machine's record is in the context. See [repo_app_removals.md](./repo_app_removals.md), "Context app lists". |
